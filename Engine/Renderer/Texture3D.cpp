...
void Texture3D::Clear() {
	glBindTexture(GL_TEXTURE_3D, m_ID);
	uint32_t z = 0;
	glClearTexImage(m_ID, 0, GL_RGBA32F, GL_UNSIGNED_BYTE, &z);
}
...
void Texture3D::ImageBind(unsigned int slot) {
	glBindImageTexture(slot, m_ID, 0, GL_TRUE, 0, GL_READ_WRITE, GL_RGBA32F);
}
void Texture3D::UnImageBind(unsigned int slot) {
	glBindImageTexture(slot, 0, 0, GL_TRUE, 0, GL_WRITE_ONLY, GL_RGBA32F);
}
