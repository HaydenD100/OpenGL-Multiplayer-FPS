...
namespace Player
{
	...
	void Player::Init() {
		...
		SceneManager::GetCurrentScene()->AddGameObject(GameObject("player", AssetManager::GetModel("player") , glm::vec3(0, 10, 5), false, 1, Capsule, 0.5, 2.2, 0.5));
		SceneManager::GetCurrentScene()->AddGameObject(GameObject("player_head", AssetManager::GetModel("player"), glm::vec3(0, 10, 5), false, 0, Sphere, 0, 0, 0));
	...
	void Player::Shoot() {
		...
		if (WeaponManager::GetGunByName(gunName)->type == Melee) {
			...
			if (hit.m_collisionObject != nullptr) {
				GameObject* gameobject = SceneManager::GetCurrentScene()->GetGameObject(hit.m_collisionObject->getUserIndex());
